...
def train(cfg, lm, tokenizer, bsz: int, embedding: str):
    lm.to(cfg.device)
    train_dl = DataLoader("data/tinystories/train.txt", tokenizer, bsz, cfg.ctx_size)
    val_dl = DataLoader("data/tinystories/validation.txt", tokenizer, bsz, cfg.ctx_size)
    ...
    log_file = open(f"log_{embedding}.txt", "w")
    log_step = 50
    evaluate_step = 500
    stop_step = 1000
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding", type=str, choices=["alibi", "rope", "pos"])
    ...
    match embedding:
        case "alibi":
            from llama_alibi import ModelConfig, GPT
            cfg = ModelConfig(
            ...
            lm = GPT(cfg)
            train(cfg, lm, tokenizer, bsz, embedding)
        
        case "pos":
            from llama_pos import ModelConfig, GPT
            cfg = ModelConfig(
                vocab_size=tokenizer.n_vocab,
                n_embd=256,
                n_heads=4,
        ...
        case "rope":
            from llama_rope import ModelConfig, GPT
            cfg = ModelConfig(
                vocab_size=tokenizer.n_vocab,
                n_embd=256,
                n_heads=4,
